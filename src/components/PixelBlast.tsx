...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const clock = { getElapsedTime: () => (performance.now() - clockStart) / 1000 };
      const setSize = () => {
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect('NoiseEffect', `uniform float uTime;uniform float uAmount;float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}void mainUv(inout vec2 uv){}void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){float n=hash(floor(uv*vec2(1920.,1080.))+floor(uTime*60.));float g=(n-.5)*uAmount;outputColor=inputColor+vec4(vec3(g),0.);}`, { uniforms: new Map([['uTime', new THREE.Uniform(0)], ['uAmount', new THREE.Uniform(noiseAmount)]]) });
